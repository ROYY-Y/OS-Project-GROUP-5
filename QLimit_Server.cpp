...
void addMail(std::shared_ptr<Client> from,std::shared_ptr<Client> target, std::string msg) {
    std::lock_guard<std::mutex> lk(target->mx);

    if (target->mail.size() >= MAX_MAIL) {
        std::string warn = "[Server Warning] :You send too many message! Please wait for messages to be delivered.\n";
        from->mail.push(warn);
        from->cv.notify_one();
        return;
    }
...
void showJoined(std::shared_ptr<Client> c, std::string rName){
    for(auto &[id, cl] : Rooms[rName]->member){
        if(id == c->id) continue;
        addMail(c,cl,"[Server] :Client " + std::to_string(c->id) + " joined " + rName + " room ...");
        cl->cv.notify_one();
    }
}
...
void removeMember(std::shared_ptr<Client> c, std::string rName){
    
    {
        ...
        for(auto &[id, cl] : Rooms[rName]->member){
                if(id == c->id) continue;
                addMail(c,cl,"[Server] :Client " + std::to_string(c->id) + " disconnected ...");
                cl->cv.notify_one();
        }
    }
...
void client_Thread(std::shared_ptr<Client> c){
    ...
    while(true){
        ...
        if(msg.rfind("JOIN ", 0) == 0){
            ...
            addMail(c,c,"[Server] :JOIN " + rName + " Successfull");
            ...
            if(c->currRoom == "Lobby"){
                addMail(c,c,"[Server] :You've already been in Lobby ...");
                c->cv.notify_one();
                continue;
            }
            removeMember(c,c->currRoom);
            addMember(c,"Lobby");
            addMail(c,c,"[Server] :Return to lobby ...");
            showJoined(c,"Lobby");
        }
            ...
            for(auto &[id, cl] : Rooms[c->currRoom]->member){
                if(id == c->id) continue;
                addMail(c,cl,"[Client " + std::to_string(c->id) + "] (All): " + sendMsg);
                cl->cv.notify_one();
            }
        }
            ...
            for(auto &[id , cl] : Rooms[c->currRoom]->member){
                if(id == toId){
                    addMail(c,cl,"[Direct message from client " + std::to_string(c->id) + "] : " + sendMsg);
                    cl->cv.notify_one();
                }
            }
        }
